...
export const maxDuration = 300; // Value in seconds (10 minutes)
        ...
        async start(controller) {
            ...
            try {
                ...
                if (!prompt) {
                    ...
                    // Close the stream here after sending error, but wrapping in setTimeout to let the chunk flush?
                    // actually safeClose outside finally or just return;
                    return;
                }
                ...
                for (let i = 1; i <= nftsToGenerate; i++) {
                    ...
                    const selectedTraits = categories.map(cat => {
                        ...
                        return categoryTraits.length > 0 ? categoryTraits[Math.floor(Math.random() * categoryTraits.length)] : null;
                    }).filter((t): t is GeneratedTrait => t !== null);
