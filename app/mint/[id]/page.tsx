...
export default function MintPage() {
    const { id } = useParams();
    const { user, authenticated, login, createWallet } = usePrivy();
    ...
    const handleMint = async () => {
        ...
        try {
            // 0. Ensure Account Exists on Movement Testnet
            if (!aptosAddress) {
                await createWallet();
            }

            // Re-check address after creation or use current
            // Note: aptosAddress from hook might not update immediately if we just created wallet
            // So we use a safe fallback or assume it will be ready for the next step 
            // Realistically we might need to wait, but let's proceed to init-account

            // We use the address we have or try to get it again (simplified for now)
            const targetAddress = aptosAddress || user?.wallet?.address;
            ...
            if (targetAddress) {
                ...
                if (!initRes.ok) console.warn("Account init warning (might already exist)");
            }

        } catch (initErr) {
            console.error("Account init failed, attempting mint anyway:", initErr);
        }
        ...
        try {
            ...
            const descriptionWithRarity = `${reveal.description || "Generated NFT"} | Rarity: ${Math.round(reveal.rarityScore || 0)}`;
            ...
            await markAsMinted(reveal.id, result.hash);
            ...
            // Robust check for EALREADY_MINTED (Move abort 0x2)
            // Error objects can be complex, so we check various properties and the full string
            const errorString = error?.toString() || "";
            const errorMessage = error?.message || "";
            const errorJson = JSON.stringify(error, Object.getOwnPropertyNames(error));

            if (errorMessage.includes("EALREADY_MINTED") || errorMessage.includes("0x2") ||
                errorString.includes("EALREADY_MINTED") || errorString.includes("0x2") ||
                errorJson.includes("EALREADY_MINTED") || errorJson.includes("0x2")) {

                console.log("⚠️ NFT already minted on-chain. Syncing database...");

                // Mark as minted in DB to prevent future selection
                await markAsMinted(reveal.id, "ALREADY_ON_CHAIN_CONFLICT");

                // Remove from local list
                setNfts(prev => prev.filter(n => n.id !== reveal.id));

                alert("This specific NFT was just claimed by someone else! Please try again to mint a different one.");
            } else {
                    ...
                    {minted && mintedNft ? (
                        ...
                        <div>
                            ...
                            >
                                ...
                                {minting ? "Minting..." : (
                                    nfts.length === 0 ? "SOLD OUT" : (authenticated ? "Mint on Testnet" : "Connect Wallet to Mint")
                                )}
                            </button>
